defmodule Tinkex.Transform do
  ...
  - `:aliases` - Map of key aliases for renaming (e.g., `%{old_key: "newKey"}`)
  - `:formats` - Map of key formatters (`:iso8601` or unary function)
  - `:drop_nil?` - Whether to drop `nil` values (default: `false`)

  ## Examples

      # Basic transformation
      iex> Transform.transform(%{temperature: 0.7, max_tokens: NotGiven.value()})
      %{"temperature" => 0.7}

      # With aliases
      iex> Transform.transform(%{temp: 0.7}, aliases: %{temp: "temperature"})
      %{"temperature" => 0.7}

      # With formatters
      iex> Transform.transform(%{created_at: ~U[2024-01-01 00:00:00Z]}, formats: %{created_at: :iso8601})
