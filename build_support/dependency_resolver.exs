unless Code.ensure_loaded?(Pristine.Build.DependencyResolver) do
  # Mix can evaluate both git subdir packages from this monorepo in the same VM.
  # Guard the helper so the shared module is defined once per session.
  defmodule Pristine.Build.DependencyResolver do
    ...
    def pristine_codegen(opts \\ []) do
      resolve(
        ...
        [github: @repo, branch: "main", subdir: "apps/pristine_codegen"],
        opts
      )
    end
    ...
    def pristine_provider_testkit(opts \\ []) do
      resolve(
        ...
        [github: @repo, branch: "main", subdir: "apps/pristine_provider_testkit"],
        opts
      )
    end
