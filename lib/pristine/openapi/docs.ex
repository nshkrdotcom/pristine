defmodule Pristine.OpenAPI.Docs do
  ...
  def build(generator_state, ir) when is_map(generator_state) and is_map(ir) do
    ...
    manifest = %{
      ...
      generated_files: rendered_files |> Enum.map(&Map.get(&1, :location)) |> Enum.sort(),
      operations:
      ...
      schemas:
        Map.get(ir, :schemas, [])
        |> Enum.filter(&documented_schema?(&1, rendered_modules))
  ...
  defp documented_schema?(schema, rendered_modules) do
    case Map.get(schema, :module_name) do
      nil -> true
      module_name -> MapSet.member?(rendered_modules, module_name)
    end
  end
  ...
  defp schema_signature_hash(schema, schemas_by_ref) do
    schema
    |> schema_signature(schemas_by_ref)
  ...
  defp schema_signature(schema, schemas_by_ref) when is_map(schema) do
    [
      ...
      fields:
        ...
        |> Enum.map(&field_signature(&1, schemas_by_ref))
        |> Enum.sort()
    ]
  end

  defp field_signature(field, schemas_by_ref) do
    [
      name: field.name,
      type: normalize_signature_term(field.type, schemas_by_ref),
  ...
  defp normalize_signature_term(reference, schemas_by_ref) when is_reference(reference) do
  ...
  defp normalize_signature_term(%_{} = struct, schemas_by_ref),
    do: struct |> Map.from_struct() |> normalize_signature_term(schemas_by_ref)

  defp normalize_signature_term(map, schemas_by_ref) when is_map(map) do
    map
    |> Enum.map(fn {key, value} ->
      {normalize_signature_term(key, schemas_by_ref),
       normalize_signature_term(value, schemas_by_ref)}
    end)
    |> Enum.sort()
  end

  defp normalize_signature_term(tuple, schemas_by_ref) when is_tuple(tuple) do
    tuple
    |> Tuple.to_list()
    |> Enum.map(&normalize_signature_term(&1, schemas_by_ref))
    |> List.to_tuple()
  end

  defp normalize_signature_term(list, schemas_by_ref) when is_list(list),
    do: Enum.map(list, &normalize_signature_term(&1, schemas_by_ref))

  defp normalize_signature_term(atom, _schemas_by_ref) when is_atom(atom),
    do: Atom.to_string(atom)

  defp normalize_signature_term(value, _schemas_by_ref), do: value
  ...
  defp rendered_file?(file) do
    case {Map.get(file, :location), Map.get(file, :contents)} do
      {location, contents} when is_binary(location) and contents not in [nil, ""] ->
